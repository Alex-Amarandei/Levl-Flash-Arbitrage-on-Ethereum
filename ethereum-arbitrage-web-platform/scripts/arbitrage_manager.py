...
from brownie import FundsManager, interface
    ...
    while True:
        try:
            with open("data/orders.json", "r") as order_book_file:
                ...
                uniswap_router_address = get_address_at(
                    name=["router", "uniswap"], source="config"
                )
                ...
                for order in order_book:
                    ...
                    stable_price_0 = get_stable_price(
                        uniswap_factory_address, token_0_address
                    )

                    if stable_price_0 == -1:
                        update_order_book(id, status="REJECTED")
                        continue

                    stable_price_1 = get_stable_price(
                        uniswap_factory_address, token_1_address
                    )
                    ...
                    (
                        ...
                        uniswap_reserves_1,
                        _,
                    ) = interface.IUniswapV2Pair(uniswap_pair_address).getReserves(
                        {"from": get_account()}
                    )
                    ...
                    (
                        ...
                        _,
                    ) = interface.IUniswapV2Pair(sushiswap_pair_address).getReserves(
                        {"from": get_account()}
                    )
                    ...
                    if from_0_to_1:
                        ...
                        difference = (
                            amount_after_selling / amount_to_borrow
                            - 1 / sushiswap_price
                        )
                    else:
                    ...
                    print(
                        f"{tag('ARBITRAGE')} {underline('Price in ETH:')} {main_price}"
                    )

                    print(
                        f"{tag('ARBITRAGE')} {underline('From 0 to 1:')} {from_0_to_1}"
                    )
                    ...
                    print(
                        f"{tag('ARBITRAGE')} {underline('Gas cost:')} {red(gas_cost)}"
                    )
                    ...
                    print(
                        f"{tag('ARBITRAGE')} {underline('Account:')} {purple(user_address)}"
                    )
                    ...
                    tx = interface.IUniswapV2Pair(sushiswap_pair_address).swap(
                        parameters[0],
                        parameters[1],
                    ...
                    update_order_book(id, hash=tx.txid, status="COMPLETED")

                    try:
                        funds_manager_contract = get_contract(
                            "FundsManager", FundsManager
                        )
                        funds_manager_contract.useGas(
                            user_address, fee * 10**18, {"from": get_account()}
                        )
                    except Exception as err:
                        print(
                            f"{tag('ARBITRAGE')} {red(f'Error transferring funds from {purple(user_address)}')}{red(', check migrations.')}"
                        )
                        print(red(err))
            ...
            print(
                f"{tag('ARBITRAGE')} {red(f'Error, marking order {purple(id)}')} {red('as rejected.')}"
            )
            print(red(err))
            update_order_book(id, status="REJECTED")
